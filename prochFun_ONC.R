...
bean.plot <- function(csv.name, keep.cite, save.plot = TRUE){
  ...
  keep = cbind(keep, LETTERS[1:length(keep.cite)])
  ...
  if(save.plot==TRUE){
    ggsave(filename = "figure1.eps", plot = beanplot, width = 4, height = 7,units = "in", device=cairo_ps)
    ggsave(filename = "figure1.png", plot = beanplot, width = 4, height = 7,units = "in", dpi = 300, type = "cairo")
...
if(save.plot==TRUE){
setEPS()
postscript("figure3.eps", horizontal = FALSE, onefile = FALSE, paper = "special", 
           height = 10, width=10)
a = plot_r(r = r, n = n, plot = plot)
dev.off()

png(filename = "figure3.png", width = 10, height = 10, units = "in", res = 300)
a = plot_r(r = r, n = n, plot = plot)
dev.off()
print("EPS and PNG files generated.")
}
...
zou.test <-function(r.jk, r.jh, r.kh, n, twotailed = TRUE,alpha = .05,
  ...
  final.plot <- ggarrange(r.plot1, r.plot2,ncol=1,nrow=2,align="hv", heights = c(2,1))
  
  if(plot==TRUE){print(final.plot)}
  
  if(save.plot==TRUE){
  ggsave(filename = "figure4.eps", plot = final.plot, width = 7, height = 5,units = "in")
  ggsave(filename = "figure4.png", plot = final.plot, width = 7, height = 5,units = "in")
  print("EPS and PNG files generated.")
  }
...
errorPlot.lazy <-function(csv.name, plotname = "figure5", save.plot=TRUE){
